...
export default function MainHeader(props: Props) {
  ...
  const updateMobileDrop = (arg) => {
    mobileDropDown.includes(arg)
      ? setMobileDropDown(mobileDropDown.filter((item) => item !== arg))
      : setMobileDropDown([...mobileDropDown, arg]);
  };

  const dropArrow =
    "https://ozksgdmyrqcxcwhnbepg.supabase.co/storage/v1/object/public/assets/4175/7233677e-c5d7-4643-96c9-213e2aec7451";
          ...
          <div className=" max-w-[1440px] h-full flex flex-row justify-between items-center gap-[30px]">
            {headerItems
              ? headerItems.map((item) => {
                return (
                  <div
                    className={`group cursor-pointer h-full flex justify-center items-center`}
                  >
                    <a
                    ...
                    {/*item?.menuDropType == "complexDrop"*/ item?.complexDrop
                        ?.firstRow?.length ||
                        item?.complexDrop?.secondRow?.lenght
                      ? (
                        <div
                          className={`z-999 w-full hidden h-fit group-hover:flex absolute top-[115px] left-0 bg-white border-b border-b-gray-300`}
                        >
                          <div className={`w-[1026px] my-8 mx-auto`}>
                                ...
                                return (
                                  ...
                                  >
                                    <div className={`h-44 overflow-hidden`}>
                                      <Image
                                        src={itemDrop.image}
                                        className={`h-full hover:scale-110 transition-all duration-[0.5s]`}
                                      >
                                      </Image>
                                    </div>
                                    <p
                                      className={`mt-3 text-center font-['lato'] font-bold text-transparent group-hover/card:text-[#FF6B00] uppercase`}
                                    >
                                      {itemDrop.text}
                                    </p>
                                  </a>
                            ...
                            <div className={`flex flex-row justify-between`}>
                              {item?.complexDrop?.secondRow?.map((itemDrop) => {
                                return (
                                  <div
                                    className={`h-24 w-80 bg-cover relative group/card`}
                                  >
                                    <Image src={itemDrop?.image}></Image>
                                    <p
                                      className={` z-999 translate-y-[-50%] translate-x-[-50%] left-1/2 text-center text-white group-hover/card:text-[#FF6B00] font-['lato'] font-bold absolute top-1/2`}
                                    >
                                      {itemDrop?.text}
                                    </p>
                                  </div>
            ...
            ? headerItems.map((item, index) => {
              ...
              return (
                <div className={``}>
                  <div
                    onClick={() => {
                      updateMobileDrop(index);
                    }}
                    className={`border-t border-gray-200 ${
                      index + 1 == mobileDropDown?.lenght ? "border-b" : ""
                    } py-4 pr-4 flex flex-row w-full justify-between items-center`}
                  >
                    <p
                      className={`font-bold text-[14px] font-['lato'] uppercase`}
                    >
                    ...
                    {item?.complexDrop?.firstRow?.length
                      ? item.complexDrop?.firstRow?.map((item, index) => {
                        return (
                          <a
                            href={item?.href}
                            className={`border-t border-gray-200 py-4 pr-4 flex flex-row w-full justify-between items-center`}
                          >
                            {item.text}
                          </a>
                        );
